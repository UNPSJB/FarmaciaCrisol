...
class AltaMedicamento(MdiWidget, Ui_vtnAltaMedicamento):
    ...
    def inicializarTabla(self):
        monodrogas = {}
        i = 0
        for instance in self.sesion.query(Monodroga).order_by(Monodroga.nombre):
    ...
    def obtenerMonodroga(self):
        ...
        query = self.sesion.query(Monodroga).filter(Monodroga.nombre == nombre)
    ...
    def confirmarOperacion(self):
        if self.validarDatos.validarCamposVacios(self.campos):
            if self.monodroga != None:
                ...
                if medicamento.guardarMedicamento(self.sesion):
...
class BajaMedicamento(MdiWidget, Ui_vtnBajaMedicamento):
    ...
    def buscarMedicamentos(self):
        ...
        query = self.sesion.query(Medicamento).filter(Medicamento.nombreComercial
                    .like('%'+str(self.lineNomb.text())+'%')).order_by(Medicamento.nombreComercial)
        i = 0
        for instance in query.all():
    ...
    def obtenerMedicamento(self):
        ...
        query = self.sesion.query(Medicamento).filter(Medicamento.nombreComercial == nombre)
        for instance in query.all():
             self.medicamento = instance
    ...
    def confirmarOperacion(self):
        if self.tablaMedicamento.rowCount() < 1:
            ...
            if self.medicamento == None:
                ...
                self.sesion.delete(self.medicamento)
                self.sesion.commit()
...
class ModificarMedicamento(MdiWidget, Ui_vtnModificarMedicamento):
    ...
    def buscarMonodroga(self):
        ...
        monodrogas = {}
        i = 0
        query = self.sesion.query(Monodroga).filter(Monodroga.nombre
                    .like('%'+str(self.lineNombMon.text())+'%')).order_by(Monodroga.nombre)
        for instance in query.all():
            item = [instance.nombre, instance.tipoVta, instance.descripcion]
            monodrogas[i] = item
            i += 1
